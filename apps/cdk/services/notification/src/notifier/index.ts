...
import { SesTemplates } from "./ses-templates/index.js";
...
export class Notifier extends Construct {
	constructor(scope: Construct, id: string, props: NotifierProps) {
		...
		[
			...
			const sesTemplates = new SesTemplates(this, "SesTemplates");
			...
			const handler = new GoFunction(this, "Handler", {
				...
				environment: {
					STAGE: props.backboneRegionalStack.stage,
					...
					INFO_SES_TEMPLATE_NAME: sesTemplates.info.ref,
					SINGLE_ACTION_SES_TEMPLATE_NAME: sesTemplates.singleAction.ref,
					PURCHASING_ORDER_SES_TEMPLATE_NAME: sesTemplates.purchasingOrder.ref,
				},
			});
			...
			handler.addToRolePolicy(
				new PolicyStatement({
					...
					resources: [
						props.backboneRegionalStack.apiGateway.arnForExecuteApi(),
					],
				}),
			);
			handler.addToRolePolicy(
				new PolicyStatement({
					...
					resources: [
						...
						`arn:aws:ses:${Stack.of(this).region}:${Stack.of(this).account}:template/*`,
					],
				}),
			);
			...
			new Rule(this, "IdentityRule", {
				...
				eventPattern: {
					...
					detailType: [IdentityEvent.UserCreatedDetailType],
				},
			...
			new Rule(this, "IdentityStaffRule", {
				...
				eventPattern: {
					...
					detailType: [IdentityStaffEvent.UserCreatedDetailType],
				},
				targets: [new SqsQueue(bufferQueue)],
			});
		});
